...
test('?gameId=<bogus> on /?view=games → "That game is no longer available" toast + param stripped', async ({
  ...
  // Toast text from src/contexts/DataContext.jsx consumeDeepLinks.
  await expect(page.getByText('That game is no longer available')).toBeVisible({
    timeout: 5_000,
  });
...
test('?groupId=<bogus> on /?view=groups → "That group is no longer available" toast', async ({
  ...
  await expect(page.getByText('That group is no longer available')).toBeVisible({
    timeout: 5_000,
  });
...
test('no false-toast on a valid view=games deep-link (no gameId param)', async ({ page }) => {
  ...
  await expect(page.getByText('That game is no longer available')).toBeHidden();
  await expect(page.getByText('That group is no longer available')).toBeHidden();
});
