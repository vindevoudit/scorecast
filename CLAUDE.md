...
| Forward roadmap                              | `C:\Users\vinde\.claude\plans\ROADMAP.md` — Tiers 4b, 7, 8.6, 10 (Tiers 5, 5.4b, 5.5, 5.5b, 6, 9, 13 shipped).                                                                                                |
...
- **E2E test DB vs prod schema (Tier 5.5b)**: `globalSetup` runs `sequelize.sync({ alter: false })` first, then `runMigrations()`. The token tables (`email_verification_tokens`, `password_reset_tokens`, `refresh_tokens`) are created by sync without `ON DELETE CASCADE` on the user FK because the Sequelize associations in [models/index.js](models/index.js) don't declare it. The shipped migrations DO declare the cascade, but they're `CREATE TABLE IF NOT EXISTS` and turn into no-ops after sync. **Net effect in tests only:** `UserService.cascadeDelete` fails with a FK violation when the target user has any token row. Production isn't affected because there `runMigrations()` creates the tables on a virgin DB. The admin-panel spec works around this by inserting temp users via direct `User.create` (no verification token) and wiping refresh-token rows before the admin UI delete fires. If you ever switch test setup to drop+migrate, this workaround can come out — and you should also add `onDelete: 'CASCADE'` to the model-level associations so sync matches the migration. **Don't call `closeDb()` in a spec's `afterAll`** — `workers: 1` means the require('models') Sequelize pool is shared across specs; closing it stalls every later spec.
