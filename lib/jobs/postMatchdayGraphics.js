...
// Partial credit a winner-only pick on `side` earns on a DRAW — mirrors
// lib/scoring.js scorePick: (drawProb × opposite-side prob / (home+away)) × 100.
// The two sides pay differently, so the recap shows both.
...
async function run() {
  ...
  // Tier 34 - football-only job. Every existing league is football, so
  // this filter is a provably identical result set today; it exists so a
  // cricket league flipped active can never reach football-data.org.
  const activeLeagues = await League.findAll({ where: { active: true, sport: FOOTBALL } });
