...
| `currentDailyStreak`     | INTEGER NOT NULL DEFAULT 0                                   | Tier 30 Phase 3 A1. Consecutive calendar days with at least one pick. Incremented by `StreakService.applyPickForUser` post-PickService transaction. Resets to 1 on clean miss; monthly auto-freeze covers one missed day per calendar month                                                                                              |
| `longestDailyStreak`     | INTEGER NOT NULL DEFAULT 0                                   | Tier 30 Phase 3 A1. High-water mark of `currentDailyStreak` across the user's lifetime. Never resets                                                                                                                                                                                                                                     |
| `lastStreakDayKey`       | VARCHAR(10) NULLABLE                                         | Tier 30 Phase 3 A1. `YYYY-MM-DD` (UTC) of most recent pick day. NULL ⇒ user has never picked. Anchor for "same day", "next day", "missed-with-freeze", "clean miss" arms in `StreakService.computeNextState`                                                                                                                             |
| `lastStreakFreezeMonth`  | VARCHAR(7) NULLABLE                                          | Tier 30 Phase 3 A1. `YYYY-MM` (UTC) of last month the monthly auto-freeze was consumed. Freeze refreshes when the calendar month rolls                                                                                                                                                                                                   |
...
**A1 — Pick streak.** Decision logic in [services/StreakService.js](services/StreakService.js); the pure `computeNextState(prev, today, monthKey)` function returns `{changed, next, milestone}` given the prior row state and the current UTC day/month keys. The state machine has four arms:

1. **Same dayKey** ⇒ no-op, idempotent for parallel picks.
2. **First-ever pick** (`lastStreakDayKey === null`) ⇒ `current = 1`, `longest = max(longest, 1)`.
3. **Adjacent day** (`gap === 1`) ⇒ `current += 1`.
4. **Gap === 2 + freeze available** ⇒ `current += 1` AND `lastStreakFreezeMonth = monthKey`. Freeze refreshes when the calendar month rolls.
5. **Otherwise** ⇒ `current = 1`. `longest` preserved.

Plus an anti-paradox guard: a "pick in the past" relative to `lastStreakDayKey` (negative gap) is a no-op rather than a state corruption (covers clock skew / manual DB fix / replica drift).

`applyPickForUser(userId)` is fired fire-and-forget from `PickService.createPick` POST-transaction (Tier 5.3 invariant — notifications and side-effect services run outside the wrapping write transaction). The 15-test unit suite in [tests/streakService.test.js](tests/streakService.test.js) locks the matrix end-to-end.

Milestones at 7 / 14 / 30 / 60 / 100 fire a `streak-milestone` push notification via the dual-update rule (`PUSH_NOTIFICATION_TYPES` in [validation/schemas.js](validation/schemas.js) + `NOTIFICATION_TYPES` in [src/components/PushSettingsPanel.jsx](src/components/PushSettingsPanel.jsx)). Deep-link `/?view=profile` follows the Tier 18 Chunk 6a convention. The flame chip in [src/components/UserMenu.jsx](src/components/UserMenu.jsx) lives between the avatar and the username (visible on mobile too — only the username text is hidden at `<sm:`); the brightness tier escalates `bg-warning/15 → /25 → /35 + shadow-led` at 7 / 14 / 30+.
...
| `streakmaster`              | Streakmaster       | longestStreak                            | 30                                  |
...
| ------- | ----------------------- | -------------------------------------------- | ------------------------------------------------------------------------ |
| `users` | `currentDailyStreak`    | INTEGER NOT NULL DEFAULT 0                   | [20260530000003](migrations/20260530000003-users-add-streak-columns.js)  |
| `users` | `longestDailyStreak`    | INTEGER NOT NULL DEFAULT 0                   | 20260530000003                                                           |
| `users` | `lastStreakDayKey`      | VARCHAR(10) NULLABLE                         | 20260530000003                                                           |
| `users` | `lastStreakFreezeMonth` | VARCHAR(7) NULLABLE                          | 20260530000003                                                           |
...
**Verification gate (entire phase)**: lint 2/2 baseline, 95/95 unit (15 new streak tests), 21/21 games API (5 new crowd-gate), 27/27 picks API, full Playwright sweeps green on each commit (auth + me + pick-and-result + notifications-badges + settings-view).
