...
| Background jobs    | **node-cron** ([lib/scheduler.js](lib/scheduler.js)) with `pg_try_advisory_lock(crc32(jobName))` for multi-replica safety. Three scheduled jobs ([lib/jobs/syncFixtures.js](lib/jobs/syncFixtures.js): daily 03:00 UTC; [lib/jobs/syncLiveScores.js](lib/jobs/syncLiveScores.js): every 60 s; [lib/jobs/reconcileInProgressGames.js](lib/jobs/reconcileInProgressGames.js): every 5 min — defensive `?ids=` sweep against upstream `?status=` filter staleness, added 2026-05-19). No-op when `NODE_ENV=test`. See §8.16 + §8.22                                                        |
...
- **[reconcileInProgressGames.js](lib/jobs/reconcileInProgressGames.js)** (added 2026-05-19 — see §8.22 postmortem) — every 5 min. Defensive sweep over every local `status='in-progress'` game with a sourceId via `?ids=` regardless of LIVE-filter membership. Closes the gap when upstream's `?status=` filter goes stale while `?ids=` remains fresh (the canonical lookup is the source of truth). Idempotent — games whose canonical state matches the local row produce `changed=false` no-ops.

Override defaults via env: `FIXTURE_SYNC_CRON='*/2 * * * *'` for dev rapid iteration; `LIVE_SCORE_SYNC_CRON='*/30 * * * * *'` for 30-s polling (note: 7-field cron format); `IN_PROGRESS_RECONCILE_CRON='*/1 * * * *'` to crank the defensive sweep to every minute (only useful in incident-response mode).
...
```
       ...
       │      (used by 1-min reconcile + 5-min defensive job)  │
       └─────────────────────┬────────────────────────────────┘
                             │
                             ▼
       ┌────────────────────────────────┐  ┌─────────────────────────────────┐
       │ lib/jobs/syncLiveScores.js     │  │ lib/jobs/                       │
       │   '* * * * *' (every minute)    │  │   reconcileInProgressGames.js   │
       │                                 │  │   '*/5 * * * *' (every 5 min)   │
...
**Cost**: $0/mo (free tier — 10 req/min budget; 1 req/min LIVE + ~0.2 req/min averaged for the 5-min `?ids=` reconcile leaves 8+ req/min for daily fixture sync + manual admin syncs).
...
**Why this wasn't caught by prior design**: the architecture assumed upstream's two endpoints share a single freshness lane. The filtered endpoint was treated as a reliable enumeration of "what's currently live"; canonical lookup was only for catching matches that had **dropped off** the filter. football-data.org's free tier breaks that assumption.
...
- **`FIXTURE_SYNC_CRON` / `LIVE_SCORE_SYNC_CRON` / `IN_PROGRESS_RECONCILE_CRON`** — (Tier 4b + 2026-05-19) cron expression overrides for the three jobs. Defaults: `'0 3 * * *'` daily, `'* * * * *'` every minute, `'*/5 * * * *'` every 5 min. Use `node-cron` 6-field syntax (or 7-field with leading seconds field). Useful for dev rapid iteration or incident-response cadence bumps.
