...
│   ├── 20260520000002-games-add-kickoff-reminder-sent-at.js  # PWA Chunk 6: games.kickoffReminderSentAt idempotency flag for the 5-min cron
...
│   ├── cricketApi.js                    # CPL auto-results: CricketData.org (CricAPI) v1 client. isConfigured / getSeriesMatches / getMatchInfo / findSeries / budgetStatus. Structurally a sibling of footballApi, with three deliberate divergences: never logs a raw URL (the key is a ?apikey= query param), inspects the response envelope because failures arrive as HTTP 200, and treats the provider's own info.hitsToday as the primary budget signal
│   ├── cricketResult.js                 # CPL auto-results: EVERY derivation, as pure functions (no DB, no network). classifyStatus / mapInningsToSides / isRainShortened / deriveAllOut / deriveWinner / buildCricketResultPayload / resolveMatches. Governing rule is refuse-rather-than-guess
│   ├── cricketTeamNames.js              # CPL auto-results: normalizeTeamName (case/accents/punctuation, & ↔ and, St ↔ Saint) + canonicalTeamName / teamsMatch / pairKey over data/cricket-team-aliases.json. Permissive identity fallback — never throws inside a cron tick
...
│   │   ├── syncCricketResults.js        # CPL auto-results, every 10 min. Cost-gated to cricket games 2–12 h past kickoff with resultSource IS NULL, so outside that window it is one COUNT and no API call. One series_info covers all 39 fixtures; one match_info per finished match (series_info never carries score[]). Writes via CricketProviderService → the unmodified CricketResultService
│   │   └── resolveCricketMatchIds.js    # CPL auto-results, daily 06:20 UTC, 1 API hit. Stamps games.providerMatchId once per game, and raises the operator alarms: unresolved (add an alias), placeholders (rename the playoff row), needsManualEntry, stranded
...
│   ├── LeaderboardService.js            # Wraps lib/leaderboardCache: buildKey(scope, {leagueId, seasonId}) → 'overall:l:<id|*>:s:<id|*>' or 'group:<groupId>:l:<id|*>:s:<id|*>'; getOverall/getOverallForViewer (Tier 8.6 masking) / getForGroup/getForGroupForViewer / getForFriends/getForFriendsForViewer (viewer + accepted friends, uncached) / invalidate('all' | key) / invalidatePrefix(prefix)
...
│   ├── CricketResultService.js          # Tier 34: T20 scorecard writer. Commits the innings columns (+ resultSource, rainAffected) in its OWN transaction and THEN calls the unmodified GameService.setResult, whose FOR UPDATE re-read observes them. Ordering is load-bearing — reversed, applyPickTransition takes its idempotency short-circuit and silently drops a correction
│   ├── CricketProviderService.js        # CPL auto-results: the DB half. resolveMatchIds (stamp providerMatchId) / countEligibleGames / listEligibleGames / applyProviderResult (schema-check → shadow-or-claim → write → release-on-failure → audit). The claim is a conditional UPDATE ... WHERE resultSource IS NULL, so admin-vs-cron is settled in the database
...
│   ├── Game.js                          # leagueId/seasonId/sourceId/status ENUM/homeScore/awayScore/kickoffTz/halfTimeReached/phase (Tier 4b); drawProbability DECIMAL(3,2) NOT NULL DEFAULT 0 + result ENUM extended to ('home','away','draw') (draw-scoring tier); sport + the T20 innings columns (Tier 34); rainAffected + providerMatchId + providerMatchResolvedAt + resultSource (CPL auto-results). timestamps:false — the table has no updatedAt, so raw SQL must never set one
...
│   │   └── LeaderboardView.jsx          # Tier 30 Phase 1 — Leaderboards surface lifted out of DashboardView. SubTabs (Overall = clean top-3 + own "your position" row / Groups / Friends = server-side `leaderboard.friends` block, viewer + accepted friends regardless of overall top-N); LeaderboardFiltersBar sits above the sub-tabs
...
7. **Leaderboard** — [routes/leaderboard.js](routes/leaderboard.js): `GET /api/leaderboard?groupId=&leagueId=&seasonId=&orderBy=&offset=&limit=` — `optionalAuth` + `publicReadLimiter`. Query validated **inline** via `leaderboardQuerySchema.safeParse(req.query)` (the shared `validate()` middleware only handles `req.body`). `LeaderboardService.getOverallForViewer`, `getForGroupForViewer`, and `getForFriendsForViewer` all apply Tier 8.6 masking before responding. The response also carries a `friends` block (viewer + accepted friends; `[]` for anon) computed by `getForFriendsForViewer` only when `req.user` is present.
...
| `getViewerFriendIdSet(viewerId)`                                                    | [lib/friends.js](lib/friends.js)                                   | One-query lookup of accepted-friend ids for a viewer; Tier 8.6 masking input + friends-leaderboard id set                                                                                                                                                                                                                                                                                                                                                                                       | `LeaderboardService.{getOverallForViewer,getForGroupForViewer,getForFriends,getForFriendsForViewer}`                                                                                                                                                                                                                                      |
...
- **[syncCricketResults.js](lib/jobs/syncCricketResults.js)** — every 10 min (CPL auto-results). **Cost-gated** to cricket games 2–12 h past kickoff with `resultSource IS NULL`, so outside a match window it is one COUNT and no outbound call. One `series_info` covers the whole competition; one `match_info` per finished match, because `series_info` never carries `score[]`. Writes through `CricketProviderService` → the unmodified `CricketResultService`. Filters `sport: CRICKET` symmetrically to the football jobs.
- **[resolveCricketMatchIds.js](lib/jobs/resolveCricketMatchIds.js)** — daily `20 6 * * *`, 1 API hit (CPL auto-results). Stamps `games.providerMatchId` once per game and raises the operator alarms: `unresolved` (add a team alias), `placeholders` (rename the playoff row), `needsManualEntry` (kickoff >12 h ago, uncaptured), `stranded` (scorecard written but points unapplied). Runs well ahead of kickoff because both of its failure modes need human action.
- **[sendKickoffReminders.js](lib/jobs/sendKickoffReminders.js)** — every 5 min (PWA Chunk 6). DB-only, no API calls. **Also self-gated** — the `Game.findAll` on the 15-20 min kickoff window returns empty most of the time and the job exits at zero cost.
...
```json
{
  ...
  "group": [...],
  "groupMeta": {...},
  "friends": [/* viewer + every accepted friend, scored + masked; [] for anon */]
}
```

`overall` array is preserved for backwards compatibility (existing frontend consumes it as a list); `overallMeta` carries the pagination context for "show all" / future pagination. `viewerRow` is always populated regardless of offset/limit so the UI can render "Your rank: 247" even when the viewer is outside the page (the **Overall** `LeaderboardCard` now renders a pinned "Your position" row from `overallMeta.viewerRow` when the viewer is below the loaded slice — mirrors `GroupLeaderboardCard`). `overallOffset` (max 10000) + `overallLimit` (max 500) accepted on the query string.

#### Friends block (regression fix, 2026-06-13)

The `friends` array is the viewer + every accepted friend, scored directly from the materialized tables via `LeaderboardService.getForFriendsForViewer` and masked per the Tier 8.6 / `listFriendsPicks` contract (self never masked; a `'private'` friend is masked; no `exemptIds`). It is **NOT** a slice of the `overall` top-N — the LeaderboardView **Friends** sub-tab used to be a client-side filter over `leaderboard.overall`, which silently dropped any friend (or the viewer) sitting below rank 50 once the user base grew past 50. `getForFriends(viewerId, {leagueId, seasonId})` resolves the id set via `getViewerFriendIdSet(viewerId)` + self, reads `UserScore` (filtered) or `UserScoreOverall` (unfiltered) for that id set (same branch as `getForGroup`), defaults missing rows to 0 pts, and sorts points DESC. No cache — the friend set is per-viewer and the read is a couple of sub-ms indexed SELECTs. The Friends sub-tab empty state is gated on the _friend_ set (not `entries.length`), since the block always contains the viewer — a friendless user sees the "add friends" nudge, not a lone self-row.
...
17. **Placeholder-game UI gate** (2026-06-01 follow-up): football-data.org returns knockout fixtures with placeholder team strings ("TBD" / "Winner of QF1" / "Loser of SF2" / "Group A 1st") until participants advance. The cascade leaves these games at the sentinel `(0.50, 0.00, 0.50)` by design — `LeagueService.ensureTeamExists` deletes literal-TBD rows on each upsert, and `PredictionService.rePredictFutureFixtures` skips a game when either team's elo is null. [src/utils/teamNames.js](src/utils/teamNames.js) `isPlaceholderGame(game)` matches the placeholder pattern `/^(tbd|winner|loser|group\s|placeholder)/i` against either team name; [src/components/GameCard.jsx](src/components/GameCard.jsx) gates `<PayoutMatrix>` AND the two pick buttons on `upcoming && !isPlaceholder`. When `upcoming && isPlaceholder`, a `role="status"` chip reading "Picks open once both teams advance" renders in their place. **Don't loosen the regex** — false-positives would hide real picks. **Don't extend backend protection unless requested** — the gate is frontend-only; a placeholder-game pick submitted via curl would still score correctly once the cascade overwrites the team names + the real result arrives, so there's no data-integrity hazard. Live + finished + halted states are unaffected — the cascade would have stamped real names by the time any TBD-vs-TBD knockout actually kicked off.
...
`listGames()` server-side gates per game: crowd attached only AFTER the match has started — `game.status !== 'scheduled'` (status flipped by the live-score/lock cron) OR `new Date(game.date) <= now` (wall-clock kickoff passed; covers the cron-lag window where a started game is still `scheduled`). **The original "OR the viewer has already picked it" reveal was removed (post-launch QoL change)** — the crowd is now hidden until kickoff for everyone, picker or not, so the `viewerId` param + per-viewer pick lookup were dropped. Below the gate the field is OMITTED from the JSON entirely — preserving the anti-bias contract even against a DevTools-savvy user.

`PickService.create` + `PickService.delete` call `GameService.invalidateCrowd(gameId)` so the count tick lands instantly instead of waiting up to 60s (still relevant once the game is past kickoff).

The frontend `CrowdMeter` (in [src/components/GameCard.jsx](src/components/GameCard.jsx)) renders a full-width row: header band ("WISDOM OF THE CROWD" + total picks count), 28 px Home/Away segmented progress bar with inline percentages baked into each segment, Home/Away label footer. Segments forced to sum to exactly 100 % via `awayPct = 100 - homePct` so rounding can't leave a sliver of background between them. No frontend change was needed for the kickoff-only tightening — it already renders nothing when `game.crowd` is absent.

Boundary tests in [tests/e2e/api/games.spec.js](tests/e2e/api/games.spec.js) lock the gating semantics: no crowd on an upcoming game (picked or not), present once kicked off (wall-clock past) + aggregates cross-user, anon hidden on upcoming, anon visible on locked/finished.
