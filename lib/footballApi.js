...
//
// Rate-limit budget: 10 req/min on the free tier. We keep an in-process
// sliding-window counter and bail out early when 9/10 is reached so ad-hoc
// admin syncs don't starve the cron job. If the upstream returns 429 we
// surface a 503-style error to the caller and let the retry happen on the
// next cron tick.
//
...
function requestsAvailable() {
  const now = Date.now();
  pruneRequestLog(now);
  return Math.max(0, FREE_TIER_LIMIT_PER_MINUTE - requestLog.length);
}
...
async function callApi(pathAndQuery) {
  ...
  // Reserve one slot from the budget. We bail at 9/10 so admin manual syncs
  // can still squeeze in even if the cron just ran.
...
async function getLiveMatches() {
  ...
  return cache.getOrBuild(
    'fd:live',
    async () => {
      const raw = await callApi('/matches?status=LIVE&status=IN_PLAY&status=PAUSED');
      return (raw.matches || []).map(normalizeFixture);
    },
    LIVE_CACHE_TTL_MS,
  );
