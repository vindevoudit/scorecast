...
async function callApi(pathAndQuery) {
  ...
  // Phase 0 P0-1 — 10s timeout via AbortController. Previously the fetch
  // could hang indefinitely on upstream slowness, blocking cron ticks and
  // admin-triggered syncs. AbortError gets mapped to the existing 502
  // football_api_unreachable so callers don't need a new branch.
  ...
  try {
    ...
    if (err?.name === 'AbortError') {
      logger.warn({ url }, 'football-data.org fetch timed out (10s)');
    } else {
  ...
  // Phase 0 P0-2 — guard against truncated/garbled JSON. Upstream sends a
  // valid HTTP 200 occasionally with an incomplete body; without the catch,
  // SyntaxError bubbled up as an uncaught crash inside the cron tick.
  try {
    return await response.json();
  } catch (err) {
    logger.error({ err, url }, 'football-data.org returned malformed JSON');
    throw new errors.AppError(502, 'football_api_bad_response', 'Upstream returned malformed JSON');
  }
}
...
function normalizeFixture(raw) {
  ...
  return {
    ...
    // Trophy Cabinet — tournament round token (GROUP_STAGE / LAST_32 /
    // LAST_16 / QUARTER_FINALS / SEMI_FINALS / THIRD_PLACE / FINAL for the
    // World Cup). Stored verbatim on games.stage; segments the cabinet.
