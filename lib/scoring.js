...
function scoreCricketBreakdown(pick, game) {
  ...
  // RAIN VOIDS BOTH RUNS LEGS. A runs prediction is made pre-match and is
  // therefore on a 20-over scale; proration above normalises every innings to
  // that scale so the comparison stays like-for-like. Under DLS that breaks —
  // a side chasing a revised target over a reduced allocation produces a total
  // no 20-over projection can fairly be measured against (54 off 46 balls
  // extrapolates to 141 from a sample the batting side never tried to
  // maximise). Voiding is the neutral answer: nobody is rewarded for landing
  // near an extrapolated number, nobody is punished for a forecast the weather
  // invalidated, and the match still pays the flat +50 winner.
  //
  // The flag is set at capture (lib/cricketResult.js isRainShortened) and
  // requires BOTH a weather marker AND an innings genuinely cut short, so a
  // rain-DELAYED match that still played its full overs scores normally.
