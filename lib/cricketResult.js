...
const RAIN_MARKER_RE = /rain|weather|wet\b|reduced|d\/?l\b|dls|duckworth/i;

/**
 * Did weather actually SHORTEN this match?
 *
 * Deliberately narrower than "the status mentions rain". Two conditions, and
 * both matter:
 *
 *  1. A weather or DLS marker in the status. This is the only place the
 *     provider ever says so — there is no structured field.
 *  2. At least one innings that ended early for a NON-DISMISSAL reason, i.e.
 *     short of the full 120 balls and not all out. That is precisely the case
 *     where lib/scoring.js would prorate, and therefore precisely the case
 *     where the total stops being comparable to a pre-match 20-over prediction.
 *
 * Requiring (2) is what stops a rain-DELAYED match that still played its full
 * twenty overs each from voiding: nothing was truncated, so normal scoring is
 * correct and voiding would silently confiscate points. Requiring (1) is what
 * stops an ordinary chase won with overs to spare from voiding — that is the
 * run-rate market working as designed, not a weather event.
 */
function isRainShortened({ statusInfo, home, away }) {
  if (!statusInfo || !RAIN_MARKER_RE.test(statusInfo.text || '')) return false;
  const truncated = (side) =>
    Boolean(side) && !side.allOut && Number(side.balls) < T20_BALLS && Number(side.balls) > 0;
  return truncated(home) || truncated(away);
}
...
function buildCricketResultPayload(game, providerMatch, { leagueCode } = {}) {
  ...
  if (statusInfo.noResult) {
    // rainAffected is inert on a null result (scorePick returns 0 and
    // effectiveRuns is never consulted) but is recorded honestly so the game
    // card and any later audit read the same story the status tells.
    const payload = mapped.ok
      ? {
          ...
          rainAffected: RAIN_MARKER_RE.test(statusInfo.text || ''),
        }
      : {
          result: null,
          home: { ...EMPTY_INNINGS },
  ...
  // Computed AFTER allOut, because "shortened by weather" specifically excludes
  // an innings that ended by dismissal.
  const rainShortened = isRainShortened({
    statusInfo,
    home: { ...home, allOut: homeAllOut.value },
    away: { ...away, allOut: awayAllOut.value },
  });
