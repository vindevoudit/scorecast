...
//
// Every 5 min, finds games kicking off in the next 15-20 minutes where:
//   - status === 'scheduled'
...
// 20-min ceiling = upper bound. With a 5-min cron cadence the [15, 20) band is
// exactly one tick wide, so every game is caught once and fires 15-20 min
// before kickoff — never the old 15-30 spread (which could fire ~30 min early
// for a kickoff sitting near the top of the band).
const LEAD_MIN = 15;
const LEAD_MAX = 20;
...
async function run() {
  ...
  for (const game of games) {
    ...
    const title = `Kickoff soon: ${game.homeTeam} vs ${game.awayTeam}`;
