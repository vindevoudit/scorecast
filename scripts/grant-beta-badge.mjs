...
//
//   node scripts/grant-beta-badge.mjs [--dry-run]
//
...
try {
  // --- Sanity check: the score reset (PL game deletion) should already be done.
  const picksLeft = await scalar('SELECT count(*)::int AS n FROM picks', tx);
  const userScoresLeft = await scalar('SELECT count(*)::int AS n FROM user_scores', tx);
  const userCount = await scalar('SELECT count(*)::int AS n FROM users', tx);

  console.log('--- pre-state ---');
  console.log('users:                 ' + userCount);
  console.log('picks remaining:       ' + picksLeft + (picksLeft === 0 ? ' (ok)' : ' (WARN)'));
  console.log(
    'user_scores remaining: ' + userScoresLeft + (userScoresLeft === 0 ? ' (ok)' : ' (WARN)'),
  );
  if (picksLeft !== 0 || userScoresLeft !== 0) {
    console.log('WARN: picks/user_scores are not empty. Delete the beta PL games in the admin');
    console.log('      panel FIRST so scores are zeroed via the service-layer cascade, then');
    console.log('      re-run this script. Proceeding with the badge work anyway.');
  }
