...
function buildOpenAPIDocument() {
  ...
  const okOrChallenge = {
    200: {
      description:
        'Authenticated (sets sc_access + sc_refresh + sc_csrf cookies) OR returns { challenge: true } when 2FA is enabled',
    },
  };
  ...
  registry.registerPath({
    ...
    tags: tags.auth,
    description:
      'Authenticate with username + password. If account has 2FA, returns { challenge: true } and issues sc_challenge cookie.',
    request: { body: { content: { 'application/json': { schema: schemas.loginSchema } } } },
    responses: okOrChallenge,
  });
  ...
  registry.registerPath({
    ...
    tags: tags.auth,
    description:
      'Complete a 2FA challenge after a successful password. Requires sc_challenge cookie + code OR recoveryCode in body.',
    request: { body: { content: { 'application/json': { schema: schemas.totpVerifySchema } } } },
    responses: noContent,
  });
  ...
  registry.registerPath({
    ...
    tags: tags.me,
    description:
      'Begin 2FA enrollment. Returns QR code data URL, secret, and one-time recovery codes.',
    security: cookieAuth,
    responses: ok,
  });
  ...
  registry.registerPath({
    method: 'post',
    path: '/api/me/2fa/confirm',
    tags: tags.me,
    description: 'Confirm enrollment by submitting a valid TOTP code; sets totpEnabledAt.',
    security: cookieAuth,
    request: { body: { content: { 'application/json': { schema: schemas.totpConfirmSchema } } } },
    responses: ok,
  });
  ...
  registry.registerPath({
    ...
    tags: tags.me,
    description: 'Disable 2FA. Requires a current code or unused recovery code.',
    security: cookieAuth,
    request: { body: { content: { 'application/json': { schema: schemas.totpVerifySchema } } } },
    responses: ok,
  });
